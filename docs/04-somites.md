...
\begin{figure}
\includegraphics[width=0.6\linewidth]{figs/somites/ali_PSM_Cab_Kaga} \caption{(ref:ali-psm-F0)}(\#fig:ali-psm-F0)
\end{figure}
...
(ref:somite-phenos) Comparisons between the F~0~, F~1~ and F~2~ generations for period intercept, mean, and unsegmented PSM area. Here, the F~0~ only includes *Cab* individuals. **A**: period intercept. **B**: period mean. **C**: unsegmented PSM area. $P$-values are derived from Kruskal-Wallis tests comparing the F~2~ individuals across microscopes. For PSM area, data was only collected for the F~2~ generation.
