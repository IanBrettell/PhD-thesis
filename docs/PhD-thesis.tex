...
Mendel's work has been distilled into what are now called \emph{Mendel's laws} (Laird and Lange 2010):

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Segregation}: One allele of each parent is randomly and independently selected, with probability \(\frac{1}{2}\), for transmission to the offspring; the alleles unite randomly to form the offspring's genotype. That is to say, after having crossed two F\textsubscript{0} inbred strains that are homozygous for different alleles (denoted as AA and aa), thus generating the F\textsubscript{1} hybrid with the genotype Aa, the resultant F\textsubscript{2} individuals from the F\textsubscript{1} intercross will have a genotypic ratio of 1:2:1 respectively for the genotypes AA (homozygous for the first F\textsubscript{0} parent's allele), Aa (heterozygous), and aa (homozygous for the second F\textsubscript{0} parent's allele).
\item
  **
\end{enumerate}
