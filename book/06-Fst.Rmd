...
For non-disease traits, the $R^2$ between the PRS and the traits have reached 24.4% for height [@yengoMetaanalysisGenomewideAssociation2018], and 12-16% for educational attainment [@okbayPolygenicPredictionEducational2022].

PRS can also be used to test whether a set of causal variants are evolving across populations or over time 
[@bergReducedSignalPolygenic2019; @edgeReconstructingHistoryPolygenic2019].
...
#### Failure to discover causal variants
...
Genes interact with the environment to create phenotypic values. Yet when it comes to the differences in mean values between groups (i.e. populations) for traits that carry social benefit -- such as intelligence -- there is ongoing debate about what proportion of the variance that we observe between populations is genetic. 

Differences in allele frequencies between populations can arise through (random) genetic drift, or be driven by selective pressures towards the trait optima in a given environment [@harpakEvolutionGroupDifferences2021].
