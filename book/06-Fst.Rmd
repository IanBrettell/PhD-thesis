...
These results show that for most traits, the $F_{ST}$ of their associated alleles tend to be low, and to differ little from their matched control SNPs. This suggests that the genetic basis of polygenic traits is generally shared across global populations. Most of these variants to date have been discovered in European populations, I conclude that the poor transferability of PGS between populations is not primarily driven by differences in trait-associated allele frequencies, but is mostly likely rather driven by differences in LD structure and environment. @wangTheoreticalEmpiricalQuantification2020 determined through simulations that differences in LD and allele frequencies between populations can explain 70-80% of the loss of PGS relative accuracy for traits like body mass index and type 2 diabetes. This analysis shows that differences in allele frequencies may not contribute so much to 

### Limitation of $F_{ST}$

As $F_{ST}$ does not take into account the effect size or the direction of effect of the trait-associated allele, for highly-polygenic traits like the ones shown here, $F_{ST}$ is almost entirely decoupled from the mean additive genetic value  between populations [@bergPopulationGeneticSignal2014a]. Therefore, even when a trait's associated alleles have low $F_{ST}$, as we see for most traits here, it does not follow that the population mean values for that trait would be the same due to genetic similarities. As demonstrated by , for complex, polygenic traits, mean trait differences can be driven by small, coordinated shifts in allele frequencies that would not be detected by $F_{ST}$

In addition, height and BMI, which have been found to be under strong differential selection, appear similar to other polygenic traits, which illustrates the limitations of this analysis [**CITE**]
