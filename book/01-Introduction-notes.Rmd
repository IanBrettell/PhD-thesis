...
## SOMITES BACKGROUND REVIEWED BY ALI

## `r colorize("Background", "3B0023", "chapter_col")`^[This Chapter describes a project carried out in collaboration with Ali Seleit and Alexander Aulehla from the Aulehla Group at EMBL Heidelberg. Drs Seleit and Aulehla performed the experiments and gathered the data; my role was to carry out the bioinformatics involved in mapping the genetic variants associated with the phenotypes of interest.]
...
```{r mouse-embryo, echo = F, fig.cap = '(ref:mouse-embryo)', out.width='100%', wrapfigure = list("L", .4)}
knitr::include_graphics(here::here("book/figs/somites/mouse_embryo_gridley.png"), auto_pdf = T)
```
...
```{r F0-Cab-Kaga-HdrR, echo = F, fig.cap = '(ref:F0-Cab-Kaga-HdrR)', out.width='100%', wrapfigure = list("L", .5)}
knitr::include_graphics(here::here("book/figs/somites/ali_period_F0_Cab_Kaga.png"), auto_pdf = T)
```

In medaka, our collaborators discovered that the southern Japanese *Cab* strain and the northern Japanese *Kaga* strain have divergent somite periodicity, where *Kaga*'s tends to be faster, and *Cab*'s slower (**Figure \@ref(fig:F0-Cab-Kaga-HdrR)**). They accordingly set up a one-way F2 cross experiment as described in Chapter \@ref(MIKK-F2-cross), using the reporter-carrying *Cab* strain and the *Kaga* strain as the parental F0 strains, in order to identify genetic loci associated with these differences in clock dynamics. 
...
```{r somite-period-ali, echo = F, fig.cap = '(ref:somite-period-ali)', out.width='100%', wrapfigure = list("R", .5)}
